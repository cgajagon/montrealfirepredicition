...
def _spatial_join_points(x_left: gpd.GeoDataFrame, x_right: gpd.GeoDataFrame, rsuffix: str) -> gpd.GeoDataFrame:
    ...
    y = gpd.sjoin(x_left, x_right, how='inner', predicate='intersects', rsuffix=rsuffix)
...
def spatial_join_incidents(incidents: pd.DataFrame, mesh: pd.DataFrame) -> pd.DataFrame:
    ...
    incidents_gdf = gpd.GeoDataFrame(incidents, geometry=gpd.points_from_xy(incidents.LONGITUDE, incidents.LATITUDE), crs=4326)

    # Spatial join points with the mesh
    incidents_grid_gdf = _intersect_mesh(incidents_gdf, square_mesh_gdf, ['INCIDENT_ID'])
...
def spatial_join_property_assessments(property_assessments: pd.DataFrame, mesh: pd.DataFrame) -> pd.DataFrame:
    ...
    # Convert to GeoDataFrames
    square_mesh_gdf =  gpd.GeoDataFrame(mesh, geometry='geometry')
    property_assessments_gdf = gpd.GeoDataFrame(property_assessments, geometry=gpd.points_from_xy(property_assessments.LONGITUDE, property_assessments.LATITUDE), crs=4326)

    # Spatial join points with the mesh
    property_assessments_grid_gdf = _intersect_mesh(property_assessments_gdf, square_mesh_gdf, ['ASSESSMENT_ID'])

    # Convert to Dataframe
    property_assessments_grid = property_assessments_grid_gdf.drop(columns=['geometry'])
...
def spatial_join_census(census: pd.DataFrame, mesh: pd.DataFrame) -> gpd.GeoDataFrame:
    ...
    # Convert to GeoDataFrames
    square_mesh_gdf =  gpd.GeoDataFrame(mesh, geometry='geometry')
    census_gdf = gpd.GeoDataFrame(census, geometry=gpd.points_from_xy(census.LONGITUDE, census.LATITUDE), crs=4326)
...
def merge_incidents_property_assessments(incidents: pd.DataFrame, property_assessments: pd.DataFrame) -> pd.DataFrame:
    ...
    # Merge tables
    merged_incidents_property_assessments = pd.concat([incidents, property_assessments], ignore_index=True, sort=False)
...
def merge_incidents_property_assessments_census(incidents_property_assessments: pd.DataFrame, census: pd.DataFrame) -> pd.DataFrame:
    ...
    # Merge tables
    merged_incidents_property_assessments_census = pd.concat([incidents_property_assessments, census], ignore_index=True, sort=False)
...
def create_input_table(table, firestations):
    ...
    table['building_age'] = np.where(table['ASSESSMENT_ID'].isna() == False, table['YEAR_CONSTRUCTION'].apply(_calculate_building_age), np.nan)

    # Calculate distance to firestation    
    table = table.merge(firestations[["FIRE_STATION_ID", "LATITUDE", "LONGITUDE"]], left_on='FIRE_STATION_ID', right_on='FIRE_STATION_ID', how='left', suffixes=("", "_firestation"))
    table['distance_to_fire_station'] = _haversine_distance(table['grid_lat'], table['grid_long'], table['LATITUDE_firestation'], table['LONGITUDE_firestation'])
    
    # Generate a flag for fire incidents 
    table['is_fire'] = _generate_flag(table['INCIDENT_CATEGORY'], ['Autres incendies', 'Incendies de bâtiments'])
